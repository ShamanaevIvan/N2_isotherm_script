...
class Isotherm:
    ...
    def isotherm_read(self):
        ...
        for i in self.analysis_log:
            if len(i) == 2:
                new_analysis_log.append(
                    [np.NaN, np.NaN, np.NaN, i[0], float(i[1])])
            else:
                new_analysis_log.append(
                    [float(i[0]), float(i[1]), float(i[2]), i[3], np.NaN])

        self.df_analysis_log = pd.DataFrame(
            new_analysis_log, columns=new_analysis_log_titles)
        log_timedelta_series = pd.to_timedelta(
            '0' + self.df_analysis_log['ELAPSED TIME (HR:MN)'] + ':00')
        ...
        final_bjh_adsorption_report = [[i[0][:-1]]+i[1:]
                                       for i in self.bjh_adsorption_report]

        self.df_bjh_adsorption = pd.DataFrame(
            final_bjh_adsorption_report, columns=final_bjh_adsorption_report_titles)
        ...
        final_bjh_desorption_report = [[i[0][:-1]]+i[1:]
                                       for i in self.bjh_desorption_report]

        self.df_bjh_desorption = pd.DataFrame(
            final_bjh_desorption_report, columns=final_bjh_desorption_report_titles)
    ...
    def plot_isotherm(self):
        rp = self.df_analysis_log['RELATIVE PRESSURE'].values
        va = self.df_analysis_log['VOL ADSORBED (cc/g STP)'].values
        plt.plot(rp[~np.isnan(rp)],
                 va[~np.isnan(va)],
                 marker='o')
    ...
    def micropore_plot(self):
        plt.plot(self.df_micropore['RELATIVE PRESSURE'].astype('float64'),
                 self.df_micropore['STATISTICAL THICKNESS,(A )'].astype(
                     'float64'),
                 marker='o')
        plt.xlabel('RELATIVE PRESSURE')
    ...
    def bjh_adsorption_plot(self):
        plt.plot(self.df_bjh_adsorption['AVERAGE DIAMETER (A)'].astype('float64'),
                 self.df_bjh_adsorption['INCREMENTAL PORE VOLUME (cc/g)'].astype(
                     'float64'),
                 marker='o'
                 )
    ...
    def bjh_desorption_plot(self):
        plt.plot(self.df_bjh_desorption['AVERAGE DIAMETER (A)'].astype('float64'),
                 self.df_bjh_desorption['INCREMENTAL PORE VOLUME (cc/g)'].astype(
                     'float64'),
                 marker='o'
                 )
